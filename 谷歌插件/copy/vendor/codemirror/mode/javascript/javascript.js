...
(function(mod) {
  if (typeof exports === 'object' && typeof module === 'object')
    // CommonJS
    mod(require('../../lib/codemirror.min'));
  else if (typeof define === 'function' && define.amd)
    // AMD
    define(['../../lib/codemirror.min'], mod);
  // Plain browser env
  else mod(CodeMirror);
})(function(CodeMirror) {
  'use strict';
  ...
  CodeMirror.defineMode('javascript', function(config, parserConfig) {
    ...
    function classBody(type, value) {
      if (
        type == 'async' ||
        (type == 'variable' && (value == 'static' || value == 'get' || value == 'set' || (isTS && isModifier(value))) && cx.stream.match(/^\s+[\w$\xa1-\uffff]/, false))
      ) {
        cx.marked = 'keyword';
