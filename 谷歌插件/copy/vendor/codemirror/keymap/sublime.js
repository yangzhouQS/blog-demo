...
(function(mod) {
  ...
  cmds.splitSelectionByLine = function(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      for (var line = from.line; line <= to.line; ++line)
        if (!(to.line > from.line && line == to.line && to.ch == 0)) lineRanges.push({ anchor: line == from.line ? from : Pos(line, 0), head: line == to.line ? to : Pos(line) });
    }
  ...
  cmds.selectLine = function(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      var range = ranges[i];
      extended.push({ anchor: Pos(range.from().line, 0), head: Pos(range.to().line + 1, 0) });
    }
  ...
  cmds.swapLineUp = function(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      var range = ranges[i],
        ...
        to = range.to().line;
      newSels.push({ anchor: Pos(range.anchor.line - 1, range.anchor.ch), head: Pos(range.head.line - 1, range.head.ch) });
