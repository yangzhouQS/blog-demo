...
var Utils = {
  ...
  // 当前页面是否是扩展内页面(可以访问 extension's localStorage)
  isExtensionPage(win) {
    if (win == null) {
      win = window;
    }
    try {
      return (win.document.location != null ? win.document.location.origin : undefined) + '/' === chrome.extension.getURL('');
    } catch (error) {}
  },
  ...
  isBackgroundPage() {
    ...
    return this.isBackgroundPage && chrome.extension.getBackgroundPage && chrome.extension.getBackgroundPage() === window;
  },
  ...
  // 例： www.csdn.net => http://www.csdn.net
  ...
  isUrl(str) {
    ...
    if (dottedParts.length > 1) {
      ...
      if ((lastPart.length >= 2 && lastPart.length <= 3) || longTlds.includes(lastPart)) {
        return true;
      }
    }
  ...
  extractQuery: (() => {
    ...
    return function(searchUrl, url) {
      ...
      try {
        return url
          .slice(searchUrl.length)
          .split(queryTerminator)[0]
          .split('+')
  ...
  haveChromeVersion(required) {
    ...
    return chromeVersion && Utils.compareVersions(chromeVersion, required) >= 0;
  },
