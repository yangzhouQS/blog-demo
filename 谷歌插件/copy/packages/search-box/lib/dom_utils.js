...
var DomUtils = {
  ...
  getVisibleClientRect(element, testChildren) {
    ...
    var isInlineZeroHeight = function() {
      ...
      const isInlineZeroFontSize = elementComputedStyle.getPropertyValue('display').indexOf('inline') === 0 && elementComputedStyle.getPropertyValue('font-size') === '0px';
      ...
      if ((clientRect.width === 0 || clientRect.height === 0) && testChildren) {
        for (let child of Array.from(element.children)) {
          ...
          if (
            ...
            !(clientRect.height === 0 && isInlineZeroHeight() && computedStyle.getPropertyValue('display').indexOf('inline') === 0)
          ) {
            continue;
          }
  ...
  isSelectable(element) {
    ...
    return (
      (element.nodeName.toLowerCase() === 'input' && unselectableTypes.indexOf(element.type) === -1) || element.nodeName.toLowerCase() === 'textarea' || element.isContentEditable
    );
  },
  ...
  simulateClick(element, modifiers) {
    ...
    // "mouseup",
    const eventSequence = ['mouseover', 'mousedown', 'click'];
    const result = [];
  ...
  simulateClickDefaultAction(element, modifiers) {
    ...
    if (newTabModifier) {
      ...
      chrome.runtime.sendMessage({ handler: 'openUrlInNewTab', url: element.href, active: shiftKey === true });
