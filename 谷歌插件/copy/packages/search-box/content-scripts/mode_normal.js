...
class NormalMode extends KeyHandlerMode {
  ...
  commandHandler({ command: registryEntry, count }) {
    // console.log("commandHandler", registryEntry)
    ...
    if (registryEntry.repeatLimit != null && registryEntry.repeatLimit < count) {
      const result = confirm(`You have asked Vimium to perform ${count} repetitions of the ` + `command: ${registryEntry.description}.\n Are you sure you want to continue?`);
      if (!result) {
        return;
      }
    }
...
var NormalModeCommands = {
  ...
  // Find.
  performFind(count) {
    // for (let i = 0, end = count; i < end; i++) { FindMode.findNext(false); }
  },

  performBackwardsFind(count) {
    // for (let i = 0, end = count; i < end; i++) { FindMode.findNext(true); }
  },
  ...
  focusInput(count) {
    ...
    console.log('recentlyFocusedElement', recentlyFocusedElement);
    if (count === 1) {
...
var textInputXPath = (function() {
  ...
  const inputElements = [
    'input[' + '(' + textInputTypes.map(type => '@type="' + type + '"').join(' or ') + 'or not(@type))' + ' and not(@disabled or @readonly)]',
    'textarea',
...
//         (computedStyle.getPropertyValue("display") === "none")) { continue; }
...
//   for (link of candidateLinks) { link.wordCount = link.innerText.trim().split(/\s+/).length; }
