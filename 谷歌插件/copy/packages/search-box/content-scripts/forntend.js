...
DomUtils.documentReady(function() {
  if (typeof global === 'undefined' || global === null) {
    return Object.assign(window, root);
  }
});

let isEnabledForUrl = false;
...
const initPageJSONFormat = function() {
  DomUtils.documentReady(function() {
    const enablePageJSONFormat = Settings.get('jsonConfig').autoFormatJSONPage;
    if (enablePageJSONFormat && JSONUtils.checkIfJson()) {
  ...
  click(event) {
    ...
    while (target) {
      ...
      if (
        ...
        (target.pathName !== document.location.pathName || target.search !== document.location.search) &&
        (['', '_self'].includes(target.target) || (target.target === '_parent' && window.parent === window) || (target.target === '_top' && window.top === window))
      ) {
...
const initializePreDomReady = function() {
  ...
  chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    // console.log("initializePreDomReady", request)
    ...
    if (!request.handler || !!request.name) {
      ...
      if (request.name !== 'userIsInteractingWithThePage') {
        if (isEnabledForUrl || (request.name && ['checkEnabledAfterURLChange', 'runInTopFrame'].includes(request.name))) {
          if (request.name) {
...
var installListeners = Utils.makeIdempotent(function() {
  ...
  // "click", "mousedown", "scroll"
  for (let type of ['keydown', 'keypress', 'keyup', 'focus', 'blur']) {
  ...
  return function() {
    if (highlightedFrameElement == null) {
      ...
      highlightedFrameElement = (function() {
        ...
        const _shadowDOM = highlightedFrameElement.attachShadow ? highlightedFrameElement.attachShadow({ mode: 'open' }) : highlightedFrameElement;

        // Inject stylesheet.
        const _styleSheet = DomUtils.createElement('style');
        _styleSheet.innerHTML = `@import url(\"${chrome.runtime.getURL('packages/search-box/content-scripts/searchIframe.css')}\");`;
        ...
        console.log(_shadowDOM, _styleSheet);
...
//     },
//     abort () {
//       if (this.isShowing()) { return this.helpUI.hide(false); }
//     },

//     toggle (request) {
//       DomUtils.documentComplete(() => {
//         if (!this.helpUI) { this.helpUI = new UIComponent("pages/help_dialog.html", "vimiumHelpDialogFrame", function () {}); }
//         return this.helpUI;
//       });

//       if ((this.helpUI != null) && this.isShowing()) { return this.helpUI.hide(); } else if (this.helpUI != null) { return this.helpUI.activate(Object.assign(request, {name: "activate", focus: true})); }
//     }
//   };
// }

initializePreDomReady();
Settings.onLoaded(initPageJSONFormat);
