        ...
        _handle_error: if (typeof action === 'undefined' || !action.length || !action[0]) {
          if (!recovering) {
            ...
            if (this.lexer.showPosition) {
              errStr = '第 ' + (yylineno + 1) + ' 行解析错误:\n' + this.lexer.showPosition();
              // + "\n期望是 " + expected.join(', ') + ", 现在为 '" + this.terminals_[symbol] + "'";
            } else {
              errStr = '第 ' + (yylineno + 1) + '行解析错误: 不期望是 ' + (symbol == 1 /*EOF*/ ? 'end of input' : "'" + (this.terminals_[symbol] || symbol) + "'");
            }
    ...
    var lexer = {
      ...
      next: function() {
        ...
        if (this._input === '') {
          ...
          this.parseError('第' + (this.yylineno + 1) + '行发生错误, 无法识别.\n' + this.showPosition(), { text: '', token: null, line: this.yylineno });
        }
      },
      lex: function lex() {
        var r = this.next();
