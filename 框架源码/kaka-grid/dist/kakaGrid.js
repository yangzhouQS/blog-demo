...
(function (global, factory) {
    ...
    function bindCellClickAction(grid, cellId, { action, mouseOver, mouseMove, mouseOut, }) {
    ...
    const isPerfectScrollbar = () => !!window.PerfectScrollbar;
    const initDocument = () => {
        let css = styleCss;
    ...
    function normalize(options) {
        ...
        if (Array.isArray(options)) {
            return options.map((e) => extend$1(e, { label: e.caption || e.label }));
        }
    ...
    class BaseCheckStyle extends BaseStdStyle {
        constructor(style = {}) {
    ...
    class CheckStyle extends BaseCheckStyle {
        static get DEFAULT() {
    ...
    class SwitchStyle extends BaseCheckStyle {
        static get DEFAULT() {
    ...
    class DrawStyle extends BaseStyle {
        constructor(style = {}) {
            super(style);
            this._appearance = style.appearance || 'none';
        }
    ...
    class ImageStyle extends BaseStdStyle {
        constructor(style = {}) {
            super(defaults(style, { textAlign: 'center' }));
    ...
    class LookupStyle extends Style {
        static get DEFAULT() {
    ...
    class RadioStyle extends BaseStdStyle {
        constructor(style = {}) {
            super(defaults(style, { textAlign: 'center' }));
    ...
    function of(columnStyle, record, StyleClassDef = Style) {
        if (columnStyle) {
    ...
    class BaseColumn {
        ...
        onDrawCell(cellValue, info, context, grid) {
            ...
            if (state.cellRange &&
                ...
                const actStyle = of(style, record, this.StyleClass);
            ...
            if (promise) {
                ...
                return promise.then((val) => {
                    ...
                    const drawInternal = () => {
                        ...
                        const actStyle = of(style, _record, this.StyleClass);
                        if (actStyle.bgColor) {
    ...
    class BranchGraphColumn extends BaseColumn {
        ...
        drawInternal(_value, context, style, helper, grid, _info) {
            ...
            const { col, row } = context;
            const field = grid.getField(col, row);
            const { timeline, branches } = (_b = (this._cache ? (_a = grid[BRANCH_GRAPH_COLUMN_STATE_ID]) === null || _a === void 0 ? void 0 : _a.get(field) : null)) !== null && _b !== void 0 ? _b : calcBranchesInfo(this._start, grid, field);
    ...
    class CheckColumn extends BaseCheckColumn {
        ...
        doDrawInternal(helper, value, context, opt) {
    ...
    class TreeColumn extends BaseColumn {
        ...
        drawInternal(value, context, style, helper, grid, info) {
            ...
            let active = false;
            const state = grid[BUTTON_COLUMN_STATE_ID$2];
            if (state) {
    ...
    class RadioColumn extends BaseColumn {
        ...
        drawInternal(value, context, style, helper, grid, _info) {
            ...
            const range = grid.getCellRange(col, row);
            const cellKey = `${range.start.col}:${range.start.row}`;
            const elapsed = (_a = grid[RADIO_COLUMN_STATE_ID$1]) === null || _a === void 0 ? void 0 : _a.elapsed[cellKey];
            const opt = {
                textAlign,
                textBaseline,
                checkColor,
     ...
     * columns
     */
    const columns = { action, type, style: style$2 };
    ...
    class Scrollbar {
        ...
        _update() {
            ...
            if (this._height > MAX_SCROLL) {
                const sbSize = style$1.getScrollBarSize();
    ...
    function _getTargetFrozenRowAt(absoluteY) {
        ...
        let top = this[_$1].scroll.top;
        const rowCount = this[_$1].frozenRowCount;
    ...
    function _getTargetFrozenColAt(absoluteX) {
        ...
        let left = this[_$1].scroll.left;
        const colCount = this[_$1].frozenColCount;
        for (let col = 0; col < colCount; col++) {
            const width = _getColWidth.call(this, col);
            const right = left + width;
            if (right > absoluteX) {
    ...
    function _getFrozenColsRect() {
        ...
        const left = this[_$1].scroll.left;
        let width = 0;
        const colCount = this[_$1].frozenColCount;
        for (let col = 0; col < colCount; col++) {
    ...
    function _drawCell(ctx, col, absoluteLeft, width, row, absoluteTop, height, visibleRect, skipAbsoluteTop, skipAbsoluteLeft, drawLayers) {
        ...
        if (drawRect.height > 0 && drawRect.width > 0) {
            ...
            try {
                ...
                const dcContext = new DrawCellContext(col, row, ctx, rect, drawRect, !!cellDrawing, this[_$1].selection, drawLayers);
    ...
    function _drawRow(ctx, initFrozenCol, initCol, drawRight, row, absoluteTop, height, visibleRect, skipAbsoluteTop, drawLayers) {
        const colCount = this[_$1].colCount;
        const drawOuter = (col, absoluteLeft) => {
            // 在数据范围之外绘制
            if (col >= colCount - 1 &&
                this[_$1].canvas.width > absoluteLeft - visibleRect.left) {
                const outerLeft = absoluteLeft - visibleRect.left;
                if (this.underlayBackgroundColor === 'transparent') {
                    ctx.clearRect(outerLeft, absoluteTop - visibleRect.top, this[_$1].canvas.width - outerLeft, height);
                }
                else {
                    ctx.save();
                    ctx.beginPath();
                    ctx.fillStyle = this.underlayBackgroundColor || '#F6F6F6';
                    ctx.rect(outerLeft, absoluteTop - visibleRect.top, this[_$1].canvas.width - outerLeft, height);
                    ctx.fill();
                    ctx.restore();
                }
            }
        ...
        if (initFrozenCol) {
            ...
            const count = this[_$1].frozenColCount;
    ...
    function _toPxWidth(grid, width) {
        ...
        const w = toPx(width, grid[_$1].calcWidthContext);
    ...
    function _getColWidthDefine(col) {
        const width = this[_$1].colWidthsMap.get(col);
        if (width) {
            return width;
        }
    ...
    function _getColWidthLimits(col) {
        const limit = this[_$1].colWidthsLimit[col];
        if (!limit) {
            return null;
        }
        const result = {};
    ...
    function _calcAutoColWidthExpr() {
        ...
        for (let col = 0; col < this[_$1].colCount; col++) {
        ...
        if (hasLimitsOnAuto.length && others.length) {
            const autoPx = _toPxWidth(this, `calc(100% - (${others.join(' + ')}))`) / autoCount;
    ...
    function _getRowHeight(row) {
        ...
        const height = this[_$1].rowHeightsMap.get(row);
        if (height) {
            return height;
        }
    ...
    function _getRowsHeight(startRow, endRow) {
        ...
        this[_$1].rowHeightsMap.each(startRow, endRow, (height) => {
    ...
    function _getScrollWidth() {
        const w = this._getColsWidth(0, this[_$1].colCount - 1);
    ...
    function _getScrollHeight() {
        const internal = this.getScrollHeightInternal(this[_$1].rowCount);
        if (typeof internal === 'number') {
            return Number(internal);
        }
        let h = this.defaultRowHeight * this[_$1].rowCount;
        this[_$1].rowHeightsMap.each(0, this[_$1].rowCount - 1, (height) => {
            h += height - this.defaultRowHeight;
        });
    ...
    function _onScroll(_e) {
        const lastLeft = this[_$1].scroll.left;
        const lastTop = this[_$1].scroll.top;
        const moveX = this[_$1].scrollable.scrollLeft - lastLeft;
        const moveY = this[_$1].scrollable.scrollTop - lastTop;
        // 保留下一个计算信息
        this[_$1].scroll = {
        ...
        if (Math.abs(moveX) >= visibleRect.width ||
            ...
            // 不同的重绘
            const context = this[_$1].context;
            if (context) {
    ...
    function _bindEvents() {
        const { handler, element, scrollable } = this[_$1];
        ...
        handler.on(element, 'mousedown', (e) => {
            ...
            if (_getResizeSelectionAt.call(this, abstractPos.x, abstractPos.y)) {
                this[_$1].selectionResizer.start(e);
            }
            else {
                const resizeCol = _getResizeColAt.call(this, abstractPos.x, abstractPos.y);
                if (resizeCol >= 0 && canResizeColumn(resizeCol)) {
                    // 改变宽度
                    this[_$1].columnResizer.start(resizeCol, e);
                }
                else {
                    // 选择
                    this[_$1].cellSelector.start(e);
                }
            }
        });
        ...
        handler.on(element, 'touchend', (e) => {
            if (longTouchId) {
                clearTimeout(longTouchId);
                this[_$1].cellSelector.select(e);
                longTouchId = null;
            }
        });
        ...
        handler.on(element, 'mousemove', (e) => {
            ...
            if (this[_$1].columnResizer.moving(e) ||
                this[_$1].selectionResizer.moving(e) ||
                this[_$1].cellSelector.moving(e)) {
                return;
            }
        ...
        handler.on(element, 'click', (e) => {
            if (this[_$1].columnResizer.lastMoving(e) ||
                this[_$1].selectionResizer.moving(e) ||
                this[_$1].cellSelector.lastMoving(e)) {
                return;
            }
        ...
        this[_$1].focusControl.onKeyDown((evt) => {
            const results = this.fireListeners(DG_EVENT_TYPE.KEYDOWN, evt);
            if (array.findIndex(results, (v) => v === false) >= 0) {
                return false;
            }
        });
        this[_$1].selection.listen(DG_EVENT_TYPE.SELECTED_CELL, (data) => {
        ...
        this[_$1].focusControl.onKeyDownMove((e) => {
            _onKeyDownMove.call(this, e);
        });
        const getCopyValue = () => {
            let copyValue = '';
            const range = this[_$1].selection.range;
        ...
        this[_$1].focusControl.onCopy((e) => {
        ...
        this[_$1].focusControl.onPaste((e) => {
            const value = e.value;
            const event = e.event;
            ...
            if (isDef(pasteValue)) {
                ...
                const { col, row } = this[_$1].selection.select;
        ...
        this[_$1].focusControl.onInput((value) => {
            const col = this[_$1].selection.select.col;
            const row = this[_$1].selection.select.row;
            this.fireListeners(DG_EVENT_TYPE.INPUT_CELL, { col, row, value });
        });
        this[_$1].focusControl.onDelete((event) => {
            const { col, row } = this[_$1].selection.select;
            this.fireListeners(DG_EVENT_TYPE.DELETE_CELL, { col, row, event });
        });
        this[_$1].focusControl.onFocus((e) => {
            this.fireListeners(DG_EVENT_TYPE.FOCUS_GRID, e);
            this[_$1].focusedGrid = true;
            const col = this[_$1].selection.select.col;
            const row = this[_$1].selection.select.row;
            this.invalidateCell(col, row);
        });
        this[_$1].focusControl.onBlur((e) => {
            this.fireListeners(DG_EVENT_TYPE.BLUR_GRID, e);
            this[_$1].focusedGrid = false;
            const col = this[_$1].selection.select.col;
            const row = this[_$1].selection.select.row;
            this.invalidateCell(col, row);
        });
    ...
    class DrawGrid extends EventTarget {
        ...
        constructor(options = {}) {
            var _a, _b;
            super();
            const { rowCount = 10, colCount = 10, frozenColCount = 0, frozenRowCount = 0, defaultRowHeight, defaultColWidth, highlightBorderWidth, font, underlayRowCount = 0, underlayColCount = 0, underlayRowHeight = 'auto', underlayColWidth = 'auto', underlayBackgroundColor, borderMode = 'none', borderColor = '', borderWidth = 0, keyboardOptions, parentElement, singleSelection, disableColumnResize, } = options;
            const protectedSpace = (this[_$1] = {});
            style$1.initDocument();
        ...
        set rowCount(rowCount) {
            ...
            if (this[_$1].selection._updateGridRange()) {
                const { col, row } = this[_$1].selection.focus;
        ...
        configure(name, value) {
            const cfg = this[_$1].config || (this[_$1].config = {});
            if (isDef(value)) {
                cfg[name] = value;
            }
            return cfg[name];
        ...
        updateSize() {
            ...
            if (!width) {
                ...
                if (parent !== null) {
                    width = parent.offsetWidth - style$1.getScrollBarSize();
                }
            }
            ...
            if (!height) {
                ...
                const parent = canvas.parentElement;
                if (parent !== null) {
                    height = parent.offsetHeight - style$1.getScrollBarSize();
                }
            }
            ...
            const sel = this[_$1].selection.select;
         ...
         */
        getMaxColWidth(col) {
            const obj = this[_$1].colWidthsLimit[col];
        ...
        setMaxColWidth(col, maxWidth) {
            const obj = this[_$1].colWidthsLimit[col] || (this[_$1].colWidthsLimit[col] = {});
            obj.max = maxWidth;
        }
        /**
        ...
        getCellsRect(startCol, startRow, endCol, endRow) {
            ...
            if (isFrozenStartCell && isFrozenStartCell.col) {
                const scrollLeft = this[_$1].scroll.left;
            ...
            if (isFrozenStartCell && isFrozenStartCell.row) {
                const scrollTop = this[_$1].scroll.top;
        ...
        isFrozenCell(col, row) {
            const { frozenRowCount, frozenColCount } = this[_$1];
        ...
        setFocusCursor(col, row) {
            const focusControl = this[_$1].focusControl;
        ...
        _updatedSelection() {
            const { focusControl } = this[_$1];
            const { col: selCol, row: selRow } = this[_$1].selection.select;
        ...
        invalidateGridRect(startCol, startRow, endCol = startCol, endRow = startRow) {
            ...
            if (invalidateTarget) {
                const { frozenColCount, frozenRowCount } = this[_$1];
        ...
        addDisposable(disposable) {
            ...
            const disposables = (this[_$1].disposables = this[_$1].disposables || []);
        ...
        _getMouseAbstractPoint(evt) {
            ...
            const rect = this[_$1].canvas.getBoundingClientRect();
            ...
            const x = clientX - rect.left + this[_$1].scroll.left;
            const y = clientY - rect.top + this[_$1].scroll.top;
            return { x, y };
        }
        ...
        _getColsWidth(startCol, endCol) {
            ...
            this[_$1].colWidthsMap.each(startCol, endCol, (width, col) => {
                w +=
        ...
        _adjustColWidth(col, orgWidth) {
            const offset = this[_$1].colWidthsOffset[col] || 0;
            const limits = _getColWidthLimits.call(this, col);
        ...
        _getVisibleRect() {
            const left = this[_$1].scroll.left;
            const top = this[_$1].scroll.top;
            const width = this[_$1].canvas.width;
            const height = this[_$1].canvas.height;
        ...
        _invalidateRect(drawRect) {
            ...
            if (ctx) {
                const visibleRect = this._getVisibleRect();
                const rowCount = this[_$1].rowCount;
                ...
                const drawGridBorder = () => {
                    ...
                    if (isGrid || isContent) {
                        let width = this[_$1].canvas.width;
                        let height = this[_$1].canvas.height;
                        if (isContent) {
                            let w = this._getColsWidth(0, this[_$1].colCount - 1) - visibleRect.left;
                            let h = _getRowsHeight.call(this, 0, rowCount - 1) - visibleRect.top;
                            width = Math.min(this[_$1].canvas.width, w);
                            height = Math.min(this[_$1].canvas.height, h);
                        }
                        ctx.save();
                        try {
                            ctx.beginPath();
                ...
                const drawOuter = (row, absoluteTop) => {
                    // 在数据范围之外绘制
                    if (row >= rowCount - 1 &&
                        this[_$1].canvas.height > absoluteTop - visibleRect.top) {
                        const outerTop = absoluteTop - visibleRect.top;
                        if (this.underlayBackgroundColor === 'transparent') {
                            ctx.clearRect(0, outerTop, this[_$1].canvas.width, this[_$1].canvas.height - outerTop);
                        }
                        else {
                            ctx.save();
                            ctx.beginPath();
                            ctx.fillStyle = this.underlayBackgroundColor || '#F6F6F6';
                            ctx.rect(0, outerTop, this[_$1].canvas.width, this[_$1].canvas.height - outerTop);
                            ctx.fill();
                            ctx.restore();
                        }
                    }
                ...
                if (initFrozenRow) {
                    ...
                    const count = this[_$1].frozenRowCount;
    ...
    class BaseCheckAction extends BaseAction$1 {
        bindGridEvent(grid, cellId) {
            const state = this.getState(grid);
    ...
    class SortHeaderAction extends BaseAction$1 {
        constructor(option = {}) {
            var _a;
            super(option);
    ...
    class BaseStyle$1 extends EventTarget {
        ...
        static get DEFAULT() {
            return defaultHeaderBaseStyle
                ...
                : (defaultHeaderBaseStyle = new BaseStyle$1());
        }
        get bgColor() {
            return this._bgColor;
        }
    ...
    class BaseStdStyle$1 extends BaseStyle$1 {
        ...
        static get DEFAULT() {
            return defaultHeaderBaseStdStyle
                ...
                : (defaultHeaderBaseStdStyle = new BaseStdStyle$1());
        }
        get textAlign() {
            return this._textAlign;
        }
    ...
    class Style$1 extends BaseStdStyle$1 {
        ...
        static get DEFAULT() {
            return defaultHeaderStyle
                ? defaultHeaderStyle
                : (defaultHeaderStyle = new Style$1());
        }
        get color() {
            return this._color;
        }
    ...
    class BaseCheckStyle$1 extends Style$1 {
        constructor(style = {}) {
            super(defaults(style, { textAlign: 'center' }));
        ...
        static get DEFAULT() {
            return defaultHeaderBaseCheckStyle
                ...
                : (defaultHeaderBaseCheckStyle = new BaseCheckStyle$1());
        }
        get uncheckBgColor() {
            return this._uncheckBgColor;
        }
    ...
    class CheckHeaderStyle extends BaseCheckStyle$1 {
        static get DEFAULT() {
    ...
    class SortHeaderStyle extends Style$1 {
        constructor(style = {}) {
            super(style);
    ...
    class SwitchHeaderStyle extends BaseCheckStyle$1 {
        static get DEFAULT() {
    ...
    class MultilineTextHeaderStyle extends Style$1 {
        static get DEFAULT() {
    ...
    class Header extends BaseHeader {
        ...
        drawInternal(value, context, style, helper, _grid, { drawCellBase, getHeaderIcon }) {
     ...
     * headers
     */
    const headers = { action: action$1, style: style$3, type: type$1 };
    ...
    class Theme {
        ...
        get underlayBackgroundColor() {
            const obj = this[_$2].obj;
            const superTheme = this[_$2].superTheme;
    ...
    function drawSwitchButton(ctx, x, y, check, animElapsedTime, { uncheckBgColor = '#FFF', checkBgColor = '#008CD6', borderColor = '#BFBFBF', btnSize = { width: 44, height: 22 }, } = {}) {
        animElapsedTime =
            animElapsedTime > 1 ? 1 : animElapsedTime < 0 ? 0 : animElapsedTime;
    ...
    const LG_EVENT_TYPE = Object.assign(Object.assign({}, DG_EVENT_TYPE), { CHANGED_HEADER_VALUE: 'changed_header_value', CHANGED_VALUE: 'changed_value', CELL_RANGE: 'cell_range', CELL_VALUE: 'cell_value' });
    ...
    const parser$1 = new PathCommandsParser();
    class Path2DShim {
        constructor(arg) {
            this._ops = [];
            if (arg === undefined) {
    ...
    function buildInlines(icons, inline) {
        ...
        // && inline.filter(il => il instanceof Inline).length <- ?
            ...
            result.push(...inline.map((il) => of$7(il)).filter(isDef));
        }
        else {
            const il = of$7(inline);
            if (il) {
                result.push(il);
            }
        }
    ...
    function getThemeColor(grid, ...names) {
        ...
        if (!isDef(gridThemeColor)) {
            // use default theme
            return getChainSafe(themes$1.getDefault(), ...names);
        }
        ...
        return ((args) => {
            ...
            // use default theme
            defaultThemeColor =
                defaultThemeColor || getChainSafe(themes$1.getDefault(), ...names);
    ...
    const SWITCH_STYLE = SwitchStyle$1.default;
    const INLINE_ELLIPSIS = of$7('\u2026');
    ...
    function _inlineRect(grid, ctx, inline, rect, col, row, { offset, color, textAlign, textBaseline, font, textOverflow, icons, }) {
        ...
        let inlines = buildInlines$1(icons, inline);
    ...
    function _multiInlineRect(grid, ctx, multiInlines, rect, col, row, { offset, color, textAlign, textBaseline, font, lineHeight, autoWrapText, lineClamp, textOverflow, icons, }) {
        ...
        if (autoWrapText || lineClamp > 0 || isAllowOverflow(textOverflow)) {
            ...
            for (let lineRow = 0; lineRow < multiInlines.length; lineRow++) {
                ...
                const buildedInline = buildInlines$1(lineRow === 0 ? icons : undefined, inline);
            ...
            grid.setCellOverflowText(col, row, false);
            buildedMultiInlines = multiInlines.map((inline, lineRow) => buildInlines$1(lineRow === 0 ? icons : undefined, inline));
        }
        let paddingTop = 0;
    ...
    function gridDrawCheckbox(ctx, rect, col, row, check, helper, { animElapsedTime = 1, uncheckBgColor = helper.theme.checkbox.uncheckBgColor, checkBgColor = helper.theme.checkbox.checkBgColor, borderColor = helper.theme.checkbox.borderColor, textAlign = 'center', textBaseline = 'middle', } = {}, positionOpt = {}) {
    ...
    function gridDrawSwitchBtn(ctx, rect, col, row, check, helper, info = {}, positionOpt = {}) {
        ...
        const draw = SWITCH_STYLE === SwitchStyle$1.default
    ...
    class GridCanvasHelper {
        ...
        getTextRect(text, context, { padding, offset = 2, textAlign = 'left', textBaseline = 'middle', font, textOverflow = 'clip', icons, } = {}) {
            ...
            if (drawRect && ctx) {
                ...
                try {
                    ...
                    ctx.textBaseline = textBaseline;
                    ctx.font = font || ctx.font;
                    let inlines = buildInlines$1(icons, text);
                    if (isAllowOverflow(textOverflow) &&
                        isOverflowInlines(ctx, inlines, rect.width)) {
        ...
        tree(text, context, { font, offset = 2, color, lineColor = this.theme.tree.lineColor, buttonColor = this.theme.tree.buttonColor, buttonBgColor = this.theme.tree.buttonBgColor, buttonBorderColor = this.theme.tree.buttonBorderColor, icons, padding, textAlign = 'left', textBaseline = 'middle', textOverflow = 'clip', treeInfo, treeNodeSpace = 0, isMultilineText = false, autoWrapText = false, lineHeight = '1em', lineClamp = 0, } = {}) {
            let rect = context.getRect();
            const col = context.col;
            const row = context.row;
            color = getGridThemeColor(this._grid, this.theme, col, row, color);
    ...
    function _getBodyCellRange(grid, col, row) {
        let cellRange = null;
        const { spanBodyOptions, layoutMap } = grid[_$3];
    ...
    function _getCellValue(grid, col, row) {
        ...
        if (row < grid[_$3].layoutMap.headerRowCount) {
            const { caption } = grid[_$3].layoutMap.getHeader(col, row);
            cellValue = typeof caption === 'function' ? caption() : caption;
        }
        else {
            const { field } = grid[_$3].layoutMap.getBody(col, row);
    ...
    function _getContentHidden(grid, col, row) {
        ...
        if (row >= grid[_$3].layoutMap.headerRowCount) {
            const column = grid[_$3].layoutMap.getBody(col, row);
    ...
    function _hasHeaderField(grid, field) {
        let has = false;
        grid[_$3].layoutMap.headerObjects.forEach((cell) => {
            if (cell.field === field) {
                has = true;
            }
        });
    ...
    function _getHeaderIcon(grid, col, row) {
        const icon = grid[_$3].layoutMap.getHeader(col, row).icon;
        if (icon == null) {
            return null;
        }
    ...
    function _getCellIcon(grid, col, row) {
        const icon = grid[_$3].layoutMap.getBody(col, row).icon;
        if (icon == null) {
            return null;
        }
    ...
    function _onDrawValue(grid, cellValue, context, { col, row }, style, draw) {
        const helper = grid[_$3].gridCanvasHelper;
    ...
    function _refreshHeader(grid) {
            ...
            layoutMap.headerObjects.forEach((cell) => {
                ...
                if (cell.style) {
                    if (cell.style instanceof BaseStyle$1) {
                        const id = cell.style.listen(BaseStyle$1.EVENT_TYPE.CHANGE_STYLE, () => {
                            grid.invalidate();
                        });
        ...
        if (!grid.hiddenHeader) {
            const headerRowHeight = grid[_$3].headerRowHeight;
    ...
    function _tryWithUpdateDataSource(grid, fn) {
        if (grid[_$3].dataSourceEventIds) {
            grid[_$3].dataSourceEventIds.forEach((id) => grid[_$3].handler.off(id));
        }
        fn(grid);
        grid[_$3].dataSourceEventIds = [
            grid[_$3].handler.on(grid[_$3].dataSource, DataSource.EVENT_TYPE.UPDATED_LENGTH, () => {
                _refreshRowCount(grid);
                grid.selection.fireSelectedEvent();
                grid.invalidate();
            }),
            grid[_$3].handler.on(grid[_$3].dataSource, DataSource.EVENT_TYPE.UPDATED_ORDER, () => {
                grid.selection.fireSelectedEvent();
                grid.invalidate();
            }),
            grid[_$3].handler.on(grid[_$3].dataSource, DataSource.EVENT_TYPE.REFRESH_DATA, () => {
                grid.selection.fireSelectedEvent();
                grid.invalidate();
            }),
        ];
    ...
    function _setRecords(grid, records = []) {
        _tryWithUpdateDataSource(grid, () => {
            ...
            const newDataSource = (grid[_$3].dataSource = CachedDataSource.ofArray(records));
        ...
        _tryWithUpdateDataSource(grid, () => {
            if (dataSource) {
                if (dataSource instanceof DataSource) {
                    ...
                    const newDataSource = (grid[_$3].dataSource = new CachedDataSource(dataSource));
                    grid.addDisposable(newDataSource);
                }
            }
            else {
    ...
    class ListGrid extends DrawGrid {
        ...
        set sortState(sortState) {
            ...
            const newState = (this[_$3].sortState = isDef(sortState)
                ? sortState
                : {
                    col: -1,
                    row: -1,
        ...
        getHeaderField(col, row) {
            const hd = this[_$3].layoutMap.getHeader(col, row);
            return hd.field;
        }
        /**
        ...
        doChangeValue(col, row, changeValueCallback) {
            if (row < this[_$3].layoutMap.headerRowCount) {
                ...
                return then(_setCellValue(this, col, row, after), (ret) => {
                    if (ret) {
                        const { field } = this[_$3].layoutMap.getBody(col, row);
        ...
        getAttachCellsAreaInternal(range) {
            ...
            if (row >= this[_$3].layoutMap.headerRowCount) {
                const column = this[_$3].layoutMap.getBody(col, row);
